...
#because when we tried to download NOAA data from the past, the data was not availiable until Sept, 2020
...
all_temp <- oxygen #left join temperature to get the date matched 

all_temp <- all_temp |>
  left_join(noaa_temp, by = c("datetime"))


all_temp$year <- year(all_temp$datetime)

#seperate out chla, oxygen, and temperature
all_temp <- subset(all_temp,year>"2020")


#temperature from Jan.1 2020 to 35 days in the future
Temp<-c(all_temp$air_temp,noaa_future_temp$air_temp)
